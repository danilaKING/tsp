...
QUESTIONS = [
    ...
    "stack": "Python",
    "difficulty": "Лёгкий",
    "text": "В чём разница между операторами == и is в Python? Когда какой использовать?",
    "answer_hint": "== сравнивает значения объектов, is проверяет, ссылаются ли переменные на один и тот же объект в памяти. is рекомендуется использовать только для сравнения с None (и иногда True/False), а == — для сравнения значений."
    },
